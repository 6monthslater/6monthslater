...
def __parse_review(page: bs4.element.Tag, reviewElem: bs4.element.Tag, region: AmazonRegion) -> Review:
    ...
    manufacturer_id_regex = re.search(r"^(?:https:\/\/www\.amazon\.ca\/stores)?\/(.[^\/]+)", manufacturer_attrs["href"]) if manufacturer_attrs else None
    manufacturer_id = manufacturer_id_regex.group(1) if manufacturer_id_regex else None
