...
def extract_keyframes(review_text_doc: Doc, review_date: datetime) -> List[Keyframe]:
   ...
   '''
    ...
    for ent in review_text_doc.ents:
        if ent.label_ == "DATE": 
            parsed_date = dateparser.parse(ent.text, settings={'RELATIVE_BASE': review_date})
            if parsed_date: 
                time_expression_data.append((parsed_date.strftime('%Y-%m-%d'), ent, extract_relevant_phrase(ent)))
            else:
                print("ERROR MOMENT PARSING DATE FROM TEXT: " + ent.text)
                return None
